...
func main() {
	...
	filePath := os.Args
	fileData := readLas.Read_file(filePath[1])
	...
	for i := 0; i < len(fileData.Points); i++ {
		vertex = append(vertex, float32((float64(fileData.Points[i].X)*0.01+float64(fileData.Info.Offset.X))-Xc)/float32(fileData.Info.Extent.MaxX-Xc))
		vertex = append(vertex, float32((float64(fileData.Points[i].Y)*0.01+float64(fileData.Info.Offset.Y))-Yc)/float32(fileData.Info.Extent.MaxY-Yc))
		vertex = append(vertex, float32((float64(fileData.Points[i].Z)*0.01+float64(fileData.Info.Offset.Z))-Zc)/float32(fileData.Info.Extent.MaxZ-Zc))
	...
	for !window.ShouldClose() {
		...
		gl.ClearDepth(1.0)
		model := mgl32.HomogRotate3D(mgl32.DegToRad(float32(yaw)), mgl32.Vec3{0, 1, 0}).Mul4(
		...
		).Mul4(mgl32.Translate3D(Tx, Ty, 0)).Mul4(mgl32.Scale3D(ScaleXY, ScaleXY, ScaleXY))
